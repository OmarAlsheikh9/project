export function totalPages(mainObject,sizePerPage) {
  return Math.ceil(mainObject.length / sizePerPage);
}
export function updatepagination(mainObject,sizePerPage,currentPage) {
  ...
  for (let page = 1, size = totalPages(mainObject,sizePerPage); page <= size; page++) {
...
export function getNewPage(pageName,currentPage,mainObject,sizePerPage) {
  ...
  value.set("lastPage", totalPages(mainObject,sizePerPage));
  // 3. click on next page
  value.set("nextPage",currentPage !== totalPages(mainObject,sizePerPage) ? currentPage + 1 : currentPage);
