...
// 1. Start Point 
// 1.1 import all functions that courses need
...
let coursesURL = "http://localhost:3000/courses",instructorsURL = "http://localhost:3000/employees",
mainObject = await getDataFromServer(coursesURL),mainObjectName="courses",
relatedObject = await getDataFromServer(instructorsURL);
// console.log(relatedObject)
// console.log(mainObject)
for (let i = 0; i < mainObject.length; i++) {
  let matchedInstructor = relatedObject.find(emp =>
    Number(emp.id) === Number(mainObject[i].instructorId)
  );

  if (matchedInstructor) {
    mainObject[i].instructorId =
      matchedInstructor.firstName + " " + matchedInstructor.lastName;
  }
}

// console.log(mainObject);
// mainObject.instructorId = relatedObject.firstName + relatedObject.lastName
// console.log(mainObject)

let sizePerPage = 10,
  currentStart = mainObject.length > 0 ? 1 : 0,
  currentEnd = currentStart + sizePerPage - 1,
  keysofCourses = Object.keys(mainObject[0]),
  pagesortedBy = keysofCourses[0],
  typeofSort = "asc",
  currentData,
  currentPage=1;
// 1.3 start point default page
makeHeaderTable(keysofCourses,mainObject[0]);
...
document.querySelector(".searchInput").addEventListener("keyup", async(event) => {
  ...
  // get updated data
  mainObject = await getDataFromServer(coursesURL);
  // filter data
...
document.querySelector(".add-std").addEventListener("click", (e)=>{
  // open manage page
  window.location.href = `../Html/manageCourses.html`;
});
